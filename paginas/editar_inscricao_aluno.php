...
<body>
    ...
    <form action="atualizar_inscricao_aluno.php" method="POST">
        <input type="hidden" name="idInscricao" value="<?php echo $idInscricao; ?>">

        <label for="aluno">Aluno:</label>
        <?php
    $sqlAlunos = "SELECT idUtilizador, nomeUtilizador FROM utilizador WHERE idUtilizador = $idAluno";
    $resultAlunos = $conn->query($sqlAlunos);

    if ($resultAlunos->num_rows > 0) {
        while ($rowAluno = $resultAlunos->fetch_assoc()) {
            echo '<input type="text" name="aluno" id="aluno" value="' . $rowAluno['nomeUtilizador'] . '" readonly><br>';
        }
    }
    ?>
        ...
        <?php
    $sqlCursos = "SELECT id_curso, titulo FROM cursos WHERE id_curso = $idCurso";
    ...
    if ($resultCursos->num_rows > 0) {
        while ($rowCurso = $resultCursos->fetch_assoc()) {
            ...
            echo '<input type="text" name="curso" id="curso" value="' . $rowCurso['titulo'] . '" readonly><br>';
        }
    }
    ?>

        <label for="data_inscricao">Data de Inscrição:</label>
        <input type="datetime-local" id="data_inscricao" name="data_inscricao" value="<?php echo $dataInscricao; ?>"
            readonly><br>
        ...
        <?php
        // Obtem o id_curso da inscrição atual
        $sqlCursoAtual = "SELECT id_curso FROM inscricoes WHERE id_inscricao = $idInscricao";
        $resultCursoAtual = $conn->query($sqlCursoAtual);
        $rowCursoAtual = $resultCursoAtual->fetch_assoc();
        $idCursoAtual = $rowCursoAtual['id_curso'];

        // Consulta para obter o status da inscrição do curso atual
        $sqlStatusInscricao = "SELECT s.id_status_inscricao, s.desc 
                            FROM status_inscricao s 
                            INNER JOIN cursos c ON s.id_status_inscricao = c.estado_curso 
                            WHERE c.id_curso = $idCursoAtual";
        ...
        if ($resultStatusInscricao->num_rows > 0) {
            while ($rowStatusInscricao = $resultStatusInscricao->fetch_assoc()) {
                ...
                echo '<input type="text" name="status_inscricao" id="status_inscricao" value="' . $rowStatusInscricao['desc'] . '" readonly><br>';
            }
        }
        ?>
