...
// Função para exibir todos os cursos do docente logado
function exibirCursos($connection, $idDocente)
{
    ...
    // Vincula o parâmetro idDocente ao marcador de posição
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo '<td>';
            echo '<form id="editar_inscricao" action="editar_curso_docente.php" method="POST" style="display:inline;">';
            echo '<input type="hidden" name="idCurso" value="' . $row['id_curso'] . '">';
            echo '<input type="submit" value="Editar">';
            echo '</form>';
            echo ' | ';
            echo '<form id="excluir_inscricao" action="excluir_curso_docente.php" method="POST" style="display:inline;">';
            echo '<input type="hidden" name="idCurso" value="' . $row['id_curso'] . '">';
...
if (isset($_SESSION["tipoUtilizador"]) && $_SESSION["tipoUtilizador"] == 2 && isset($_SESSION["idUtilizador"])) {
    $idDocente = $_SESSION["idUtilizador"];
    exibirCursos($conn, $idDocente);
} else {
    echo '<script> alert("UTILIZADOR NÃO AUTORIZADO: CONTACTE O ADMIN OU ENTRE COM OUTRO UTILIZADOR!")</script>';
    echo '<meta http-equiv="refresh" content="0; url=home.php">';
}
...
?>
    ...
    <button onclick="location.href='adicionar_cursos_docente.php'">Adicionar Cursos</button>
    <div id="footer">
